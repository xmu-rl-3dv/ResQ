...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mac_out = th.stack(mac_out, dim=1)  # Concat over time [batch_size, times, n_agents, n_actions]

        # Pick the Q-Values for the actions taken by each agent
        chosen_action_qvals = th.gather(mac_out[:, :-1], dim=3, index=actions).squeeze(3)  # Remove the last dim  [batch_size, times, n_agents, n_actions]
        ...
        mask = mask.expand_as(td_error) #在qmix里面mask的长度在expand_as之前和td_error的长度是一样的。
