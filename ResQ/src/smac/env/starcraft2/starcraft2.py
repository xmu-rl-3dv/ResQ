...
class StarCraft2Env(MultiAgentEnv):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (
                    ...
                    # udpate 20200602: add angle info at [4]
                    # angle = math.degrees(math.atan2(e_y - y,e_x - x))
                    # if angle<0:
                    #     angle+=360
                    # enemy_feats[e_id, 4] = angle
                    # ind = 5
    ...
    def get_obs(self):
        ...
        # update 20200526
        # concate agents and enemies obs
        # enemies_obs = self.get_obs_e()

        return agents_obs 
        # return agents_obs + enemies_obs
    ...
    def get_obs_enemy(self, enemy_id):
        ...
        move_feats = np.zeros(move_feats_dim, dtype=np.float32)
        enemy_feats = np.zeros((enemy_feats_dim[0]-1, enemy_feats_dim[1]), dtype=np.float32)
        ally_feats = np.zeros((ally_feats_dim[0]+1, ally_feats_dim[1]), dtype=np.float32)
        own_feats = np.zeros(own_feats_dim-self.shield_bits_ally+self.shield_bits_enemy, dtype=np.float32)
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            # Enemy features            
            en_ids = [
                en_id for en_id in range(self.n_enemies) if en_id != enemy_id
            ]
            for e_id, en_id in enumerate(en_ids):
                e_unit = self.enemies[en_id]
            ...
            for i, al_id in enumerate(al_ids):
                ...
                if (
                    ...
                    avail_actions_e = self.get_avail_agent_actions(al_id)
                    ally_feats[i, 0] = avail_actions_e[
                        self.n_actions_no_attack + enemy_id
                    ]  
                    ally_feats[i, 1] = dist / sight_range  # distance
                    ally_feats[i, 2] = (al_x - x) / sight_range  # relative X
                    ally_feats[i, 3] = (al_y - y) / sight_range  # relative Y
    ...
    # add get_obs_e
    def get_obs_e(self):
        """Returns all enemy observations in a list.
        NOTE: Enemies should have access only to their local observations
        during decentralised execution.
        """
        enemies_obs = [self.get_obs_enemy(i) for i in range(self.n_enemies)]
        return enemies_obs
    ...
    def get_obs_enemy_feats_size(self):
        ...
        """
        # update 20200602: add angle info
        # nf_en = 4 + self.unit_type_bits + 1
        ...
        if ally:  # use new SC2 unit types
            ...
            if self.map_type == "stalkers_and_zealots":
                ...
                if unit.unit_type == 51:
                    ...
                    type_id = 2
            elif self.map_type == "marine_tank":
                # print(unit.unit_type, unit.health_max)
                if unit.unit_type == 32 or unit.unit_type == 33: #tank , 175 hp
                    type_id = 0
                elif unit.unit_type == 48: #marine 45 hp
                    type_id = 1
        return type_id
